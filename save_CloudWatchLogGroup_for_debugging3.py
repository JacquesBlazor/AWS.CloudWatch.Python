    ...
    for i_item in log_events_response['events']:  # 這是每個 log stream 裡的不同階段的 aws events
                ...
                if len(splited_msg)>3 and splited_msg[1]=='RequestId:' and len(splited_msg[2])==36 and splited_msg[2][8] == '-' and splited_msg[2][13] == '-' and splited_msg[2][18] == '-' and splited_msg[2][23] == '-': # 這個 START 開頭的事件, 接著會有一個 36 個字元的字串代表 RequestId 並且中間有 - 符號分隔
                    ...
                    if request_id not in listOfLogStreamName[stream]:  # 如果目前還沒有這筆 RequestId 的資料
                        ...
                        if len(splited_msg)==5 and request_id and splited_msg[3]=='Version:' and listOfLogStreamName[stream][request_id]:
                            listOfLogStreamName[stream][request_id]['version'] = splited_msg[4]  # lambda function 的版本名稱
                        target = listOfLogStreamName[stream][request_id]['lambda_events'] # 然後把 target 指向接下來同一筆 RequestId 的所有事件 event 記錄的目標位置
                        continue
                    else:
        ...
        for element in splited_msg:
            if element:
                ...
                elif msgElement.startswith('lambda_event:'):
                    meta['msg_lambda_event'] = ast.literal_eval(msgElement.lstrip('lambda_event: '))
                elif msgElement.startswith('payload:'):
                    msg_payload = msgElement.lstrip('payload: ')
                    msg_payload = msg_payload[:msg_payload.rfind('}')+1]
                    meta['msg_payload'] = ast.literal_eval(msg_payload)
                elif msgElement.startswith('event:'):
                    meta['msg_event'] = ast.literal_eval(msgElement.lstrip('event: '))
        ...
        if meta:
            if isinstance(target, list):
                if 'msg_content' in meta and len(meta['msg_content']) == 1:
                    meta['msg_content'] = meta['msg_content'][0]
                target.append(meta)
            else:
