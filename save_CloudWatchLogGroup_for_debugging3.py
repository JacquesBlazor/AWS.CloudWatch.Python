...
log_streams_response = client.describe_log_streams(logGroupName=logGroupName, orderBy='LogStreamName', descending=True, limit=5) # 只蒐集最近幾(5)次的 Log Stream
            ...
            if splited_msg[0]=='START':  # 如果這個 aws event 是 START 開頭, 代表這是 aws lambda 被 trigger 的開頭
                if len(splited_msg)>3 and splited_msg[1]=='RequestId:' and len(splited_msg[2])==36 and splited_msg[2][8] == '-' and splited_msg[2][13] == '-' and splited_msg[2][18] == '-' and splited_msg[2][23] == '-': # 這個 START 開頭的事件, 接著會有一個 36 個字元的字串代表 RequestId 並且中間有 - 符號分隔
                    ...
                    if request_id not in listOfLogStreamName[stream]:  # 如果目前還沒有這筆 RequestId 的資料
                        ...
                        listOfLogStreamName[stream][request_id]['start_timestamp'] = datetime.datetime.fromtimestamp(i_item['timestamp'] //1000).strftime('%Y-%m-%d, %H:%M:%S')
                        listOfLogStreamName[stream][request_id]['start_ingestionTime'] = datetime.datetime.fromtimestamp(i_item['ingestionTime'] //1000).strftime('%Y-%m-%d, %H:%M:%S')
                ...
                if len(splited_msg)==3 and splited_msg[1]=='RequestId:' and len(splited_msg[2])==36 and splited_msg[2][8] == '-' and splited_msg[2][13] == '-' and splited_msg[2][18] == '-' and splited_msg[2][23] == '-':
                    if request_id:
                        if splited_msg[2] == request_id:
                            ...
                            listOfLogStreamName[stream][request_id]['end_timestamp'] = datetime.datetime.fromtimestamp(i_item['timestamp'] //1000).strftime('%Y-%m-%d, %H:%M:%S')
                            listOfLogStreamName[stream][request_id]['end_ingestionTime'] = datetime.datetime.fromtimestamp(i_item['ingestionTime'] //1000).strftime('%Y-%m-%d, %H:%M:%S')
                            continue
                else:
        ...
        for element in splited_msg:
            if element:
                ...
                if msgElement.startswith('[') and msgElement.endswith(']'):
                    ...
                    meta['datetime'] = datetime.datetime.strptime(msgElement, '%Y-%m-%dT%H:%M:%S.%fZ').strftime('%Y-%m-%d, %H:%M:%S')
...
with open('%s_%d.json' % (filenameprefix, abs(datetime.datetime.now().timestamp()*1000*1000)), 'w', encoding='utf-8-sig') as f:
